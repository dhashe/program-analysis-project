...
let solver : Z3.Solver.solver option ref = ref None
...
let try_constraints s constraints =
  ...
  let () = Z3.Solver.push s in
  let () = Z3.Solver.add s constraints in
  let res = Z3.Solver.check s [] in
  (* let () = print_string "Before getting model" in *)
  let model = if res = Z3.Solver.SATISFIABLE then Z3.Solver.get_model s else None in
  let () = Z3.Solver.pop s 1 in
