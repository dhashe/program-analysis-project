  ...
  | (nxx as t)".sym"
    { let open Source in
      SYM (numop t
               ...
               (fun s -> let x = Concreteness.Symbolic
                             (Z3.FloatingPoint.mk_const Concreteness.ctx
                                (Z3.Symbol.mk_string Concreteness.ctx s.it) Concreteness.fp_sort32) in Values.F32 x)
               (fun s -> let x = Concreteness.Symbolic
                             (Z3.FloatingPoint.mk_const Concreteness.ctx
                                (Z3.Symbol.mk_string Concreteness.ctx s.it) Concreteness.fp_sort64) in Values.F64 x))
    }
  | "funcref" { FUNCREF }
  | "mut" { MUT }
